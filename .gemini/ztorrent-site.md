...
### 3.5 API 接口规范 (API Standards)

- **请求方法**: 所有业务接口均强制使用 **POST** 请求。
- **参数传递**: 所有参数必须通过 **Request Body** 传递 (DTO)。
- **响应格式**: 必须严格遵守统一响应结构 `{ code, message, data, path, timestamp }`。

**1. 成功响应规范**:

- **Code**: `1000`
- **JSON 结构**: `{ "code": 1000, "message": "ok", "data": T, ... }`
- **处理**: 拦截器校验 `code === 1000`，否则视为业务异常。

**2. 失败响应规范**:

- **Code**: `9xxx` (如 `9400` 参数错误, `9401` 未鉴权, `9500` 系统错误)
- **Data**: `{ "description": "原始错误信息" }`
- **处理**: 拦截器自动抛出异常并提示（优先使用 `message`）。
...
- ✅ 成功提示
- ✅ 自定义回调函数
- ⚠️ **错误提示由 Axios 响应拦截器统一处理**，默认不重复显示

#### 错误提示机制

项目使用 **Axios 响应拦截器** (`src/api/axiosInterceptors.ts`) 统一处理错误提示：

- 所有 API 请求错误自动弹出 Sonner Toast。
- **业务错误**：若 HTTP 200 但 `code !== 1000`，拦截器视作失败并抛出异常。
- 错误消息优先级：后端 `message` > HTTP/业务状态默认描述 > 通用消息。
- 支持静默模式：通过请求配置 `meta.silent` 禁用 toast。
