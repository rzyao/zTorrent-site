...
项目中有三种按钮类型，参考 `HomeActionBar.tsx`：
...
| 用途      | 渐变                            | 阴影                  |
| --------- | ------------------------------- | --------------------- |
| 默认/CTA  | `from-amber-500 to-orange-500`  | `shadow-amber-500/30` |
| 上传/确认 | `from-green-500 to-emerald-500` | `shadow-green-500/30` |
| 危险操作  | `from-red-500 to-rose-500`      | `shadow-red-500/30`   |

#### 2. 次要按钮（预定义类 `.general-button`）
...
// 等效的 Tailwind 类
<Button className="flex items-center gap-2 px-4 py-2 rounded-lg whitespace-nowrap transition-all border bg-linear-to-r from-amber-500/20 to-orange-500/20 border-amber-500/50 text-amber-300 hover:from-amber-500/30 hover:to-orange-500/30">
```

#### 3. Ghost 按钮（透明背景 + hover 变色）

用于工具栏、操作栏等场景，默认透明，hover 时显示颜色：

**基础结构**：

```tsx
<Button className="transition-all duration-300 px-4 py-2 h-auto border bg-neutral-900/50 border-neutral-700/50 text-gray-300 hover:bg-neutral-800/70 hover:border-{color}-500/50 hover:text-{color}-400 hover:shadow-lg hover:shadow-{color}-500/20">
```

**颜色变体速查**：

| 颜色 | hover:border    | hover:text   | hover:shadow    | 用途          |
| ---- | --------------- | ------------ | --------------- | ------------- |
| 琥珀 | `amber-500/50`  | `amber-400`  | `amber-500/20`  | 默认/导航     |
| 蓝色 | `blue-500/50`   | `blue-400`   | `blue-500/20`   | 前进后退/通知 |
| 红色 | `red-500/50`    | `red-400`    | `red-500/20`    | 热门/收藏     |
| 绿色 | `green-500/50`  | `green-400`  | `green-500/20`  | 做种/成功     |
| 紫色 | `purple-500/50` | `purple-400` | `purple-500/20` | 论坛/社区     |
| 粉色 | `pink-500/50`   | `pink-400`   | `pink-500/20`   | 活动/特惠     |
| 黄色 | `yellow-500/50` | `yellow-400` | `yellow-500/20` | 排行/VIP      |
| 青色 | `cyan-500/50`   | `cyan-400`   | `cyan-500/20`   | 用户/在线     |
| 橙色 | `orange-500/50` | `orange-400` | `orange-500/20` | 刷新/警告     |
| 靛蓝 | `indigo-500/50` | `indigo-400` | `indigo-500/20` | 书签/收藏夹   |

**完整示例**（琥珀色 Ghost 按钮）：

```tsx
<Button className="h-auto border border-neutral-700/50 bg-neutral-900/50 px-4 py-2 text-gray-300 transition-all duration-300 hover:border-amber-500/50 hover:bg-neutral-800/70 hover:text-amber-400 hover:shadow-lg hover:shadow-amber-500/20">
...
<Button
  disabled={!canGoBack}
  className="... disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:border-neutral-700/50 disabled:hover:text-gray-300 disabled:hover:shadow-none"
>
```
...
| 类型           | 内边距      | 图标尺寸  | 适用场景             |
| -------------- | ----------- | --------- | -------------------- |
| **纯图标按钮** | `px-3 py-2` | `w-5 h-5` | 导航按钮、工具按钮   |
| **图标+文字**  | `px-4 py-2` | `w-4 h-4` | 功能按钮、操作按钮   |
| **大按钮**     | `px-6 py-6` | `w-5 h-5` | 主要 CTA、详情页操作 |

**纯图标按钮**（工具栏）：

```tsx
<Button className="h-auto px-3 py-2 ... ...">
...
```tsx
<Button className="h-auto px-6 py-6 ... ...">
