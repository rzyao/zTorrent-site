      ...
      <div className="bg-[#0F171E] border-2 border-amber-500/30 rounded-xl max-w-3xl w-full max-h-[90vh] overflow-hidden shadow-2xl">
                ...
                {announcements.map((_, index) => (
                  <button
                    ...
                    className={`w-2 h-2 rounded-full transition-all ${
                      index === currentIndex ? 'bg-amber-400 w-6' : 'bg-amber-400/30 hover:bg-amber-400/50'
                    }`}
                  />
                ))}
              </div>
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
          ...
          <div className="prose prose-invert prose-amber max-w-none">
            <div className="text-amber-200/80 whitespace-pre-wrap">{currentAnnouncement.content}</div>
          </div>
        </div>
