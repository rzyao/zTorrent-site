...
export function MovieList({ movies, filtered, searchQuery, onSearchChange, selectedMovie, onSelectMovie, onClearModes }: MovieListProps) {
  return (
      ...
      <div className="p-4 space-y-2 max-h-[calc(100vh-300px)] overflow-y-auto scrollbar-themed">
        {filtered.length === 0 ? (
          <div className="text-center py-12">
            <span className="w-12 h-12 text-neutral-600 mx-auto mb-3">暂无影片</span>
          ...
          filtered.map((movie) => (
            <div
              key={movie.id}
              onClick={() => { onSelectMovie(movie); onClearModes(); }}
            ...
            >
              <div className="flex gap-3">
                <img src={movie.poster || undefined} alt={movie.title} className="w-16 h-24 rounded-lg object-cover" />
                <div className="flex-1 min-w-0">
                  <h3 className="text-white text-sm mb-1 truncate">{movie.title}</h3>
                  <p className="text-neutral-400 text-xs mb-2 truncate">{movie.originalTitle}</p>
                  <div className="flex items-center gap-2 mb-2">
                    <Badge className="bg-amber-500/20 text-amber-400 text-xs">{movie.category}</Badge>
                    <span className="text-neutral-500 text-xs">{movie.year}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-amber-400 text-xs">{movie.rating}</span>
                    <span className="text-neutral-500 text-xs">{movie.torrents.length} 个版本</span>
                  </div>
                </div>
              </div>
            </div>
