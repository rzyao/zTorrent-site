  ...
  return (
    <div className="min-h-screen bg-[#0F171E]">
      <div className="max-w-[1600px] mx-auto p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
          <div className="bg-gradient-to-br from-amber-500/10 to-orange-600/10 border border-amber-500/20 rounded-xl p-5">
            ...
            <div className="text-3xl text-amber-400 mb-1">{data.stats.pending}</div>
            <div className="text-xs text-neutral-500">影片 {data.stats.pendingMovies} · 片单 {data.stats.pendingPlaylists} · 种子 {data.stats.pendingTorrents}</div>
          </div>
        ...
        <ReviewTable
          ...
          onViewHistory={(item) => { actions.setSelectedItem(item); actions.setShowHistory(true); }}
        />
      </div>
      ...
      <HistoryModal
        ...
        onClose={() => { actions.setShowHistory(false); actions.setSelectedItem(null); }}
      />
    </div>
  );
}
