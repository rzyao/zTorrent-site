          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              <div className={`border rounded-lg p-4 ${item.submitterReputation >= 90 ? 'bg-green-500/10 border-green-500/30' : item.submitterReputation >= 70 ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
                <div className="flex items-start gap-3">
                  <User className={`w-5 h-5 shrink-0 mt-0.5 ${item.submitterReputation >= 90 ? 'text-green-400' : item.submitterReputation >= 70 ? 'text-yellow-400' : 'text-red-400'}`} />
                  <div>
        ...
        {item.status === 'pending' && (
          <div className="sticky bottom-0 bg-neutral-900 border-t border-neutral-700 p-6">
            <div className="flex gap-3">
              <button onClick={() => onApprove(item)} className="flex-1 py-3 bg-linear-to-r from-green-500 to-emerald-600 text-white rounded-lg hover:shadow-lg hover:shadow-green-500/25 transition-all flex items-center justify-center gap-2">
                <Check className="w-5 h-5" />
                通过审核
              </button>
              <button onClick={() => onReject(item)} className="flex-1 py-3 bg-linear-to-r from-red-500 to-rose-600 text-white rounded-lg hover:shadow-lg hover:shadow-red-500/25 transition-all flex items-center justify-center gap-2">
                <X className="w-5 h-5" />
                驳回申请
              </button>
            </div>
