import { Play, Star, BookmarkPlus, Users, Clock, Eye, Film } from 'lucide-react';
    ...
    >
      ...
      <div className="relative aspect-[2/3] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3">
          <h3 className="text-white text-sm mb-1 line-clamp-1 group-hover:text-amber-400 transition-colors">{movie.title}</h3>
          <p className="text-neutral-400 text-xs line-clamp-1">{movie.originalTitle}</p>
        </div>
      </div>
      ...
      <div className="p-3 md:p-4 space-y-2 md:space-y-3">
        ...
        <div className="flex flex-wrap gap-1 md:gap-1.5">
          {(movie.genre || []).slice(0, 3).map((g, index) => (
            <span key={index} className="px-1.5 md:px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-400 text-[10px] md:text-xs border border-amber-500/20">
              {g}
            </span>
          ))}
        </div>
