...
interface MovieGridProps {
  movies: FilmCardData[];
  onOpen: (movie: FilmCardData) => void;
  ...
  return (
    <>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-6">
        {movies.map((movie) => (
          <MovieCard key={movie.id} movie={movie} onClick={onOpen} onToggleCollect={onToggleCollect} />
        ))}
      </div>
      ...
      {movies.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-neutral-500 mb-6">没有找到符合条件的影片</p>
        </div>
      )}
    </>
  );
