...
export function SongsSection({
  ...
  const Row = ({ index, style }: { index: number; style: React.CSSProperties }) => {
    ...
    return (
      <div
        style={style}
        className="flex items-center gap-4 p-4 hover:bg-neutral-800/50 transition-all cursor-pointer border-b border-neutral-700/30 last:border-0 box-border"
      >
        ...
        <span className="text-neutral-500 text-sm w-32 truncate hidden md:block">{song.album}</span>
        <span className="text-neutral-500 text-sm w-16 text-right">{song.duration}</span>
        <div className="flex items-center gap-2 ml-4">
          ...
          <button
            ...
            className="p-2 rounded-lg text-neutral-400 hover:text-amber-400 hover:bg-amber-500/20 transition-all"
          >
            <Plus className="w-4 h-4" />
          </button>
        </div>
  ...
  return (
    <div className="bg-linear-to-br from-neutral-800/40 to-stone-900/40 rounded-xl border border-neutral-700/50 overflow-hidden h-[600px]">
      <AutoSizer>
        {({ height, width }) => (
          <List
            height={height}
