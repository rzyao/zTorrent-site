...
import { formatBytes, formatSpeed, getStatusBadgeClass, getStatusText, getTypeColor, getTypeIcon } from '../utils';
...
export function DownloaderCard({ downloader, onTestConnection, onFetchInfo, onEdit, onDelete }: Props) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-neutral-400 text-sm">{downloader.type}</span>
              {downloader.version && (
                <>
                  <span className="text-neutral-600">•</span>
                  <span className="text-neutral-500 text-sm">{downloader.version}</span>
                </>
              )}
            </div>
          </div>
        </div>
        <span className={`px-3 py-1 rounded-lg text-xs flex items-center gap-1 ${getStatusBadgeClass(downloader.status)}`}>
      ...
      {isConnected && (
        <div className="grid grid-cols-2 gap-3 mb-4">
          ...
          <div className="bg-neutral-800/30 rounded-xl p-3">
            ...
            <p className="text-white">{downloader.activeTorrents} / {downloader.totalTorrents}</p>
          </div>
          <div className="bg-neutral-800/30 rounded-xl p-3">
            <div className="flex items-center gap-2 mb-1">
