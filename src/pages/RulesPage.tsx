...
const getIconBySection = (id: string) => {
  const icons: { [key: string]: React.ReactNode } = {
    general: <BookOpen className="w-5 h-5" />,
    download: <Download className="w-5 h-5" />,
    upload: <Upload className="w-5 h-5" />,
    points: <Award className="w-5 h-5" />,
    vip: <Gift className="w-5 h-5" />,
    community: <Users className="w-5 h-5" />,
    violations: <AlertTriangle className="w-5 h-5" />,
    special: <Info className="w-5 h-5" />,
  };
...
export function RulesPage() {
  ...
  const [isAdmin] = useState(true); // 模拟管理员权限，实际应用中从用户上下文获取
  ...
  const deleteItem = (sectionId: string, contentIndex: number, itemIndex: number) => {
    setRuleSections((prev) =>
      prev.map((section) => {
        if (section.id === sectionId) {
          const newContent = [...section.content];
          const items = (newContent[contentIndex].items || []).filter(
            (_, i) => i !== itemIndex
          );
          newContent[contentIndex] = {
            ...newContent[contentIndex],
                        ...
                        <div key={contentIndex}>
                          ...
                          <div className="relative">
                            {isEditMode && (
                              <button
                                onClick={() =>
                                  deleteContent(section.id, contentIndex)
                                }
                              ...
                              <ul className="space-y-2 mb-3">
                                ...
                                {isEditMode && (
                                  <li className="flex items-start gap-3">
                                    <button
                                      onClick={() =>
                                        addItem(section.id, contentIndex)
                                      }
