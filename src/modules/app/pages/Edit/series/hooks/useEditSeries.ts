...
import type {
  ...
  Episode,
  SeriesTorrent,
} from "@/modules/app/pages/Edit/series/types";
...
export function useEditSeries() {
  ...
  const handleSaveSeries = async () => {
    ...
    try {
      ...
      // Refresh list
      const respList = await SeriesService.seriesBaseControllerList({
        page: 1,
        limit: 50,
        keyword: searchQuery,
      });
      const list =
        respList?.message === "ok" ? respList.data?.items : (respList as any)?.items || [];
  ...
  async function fetchEpisodes(seriesId: string) {
    try {
      const resp = await EpisodesService.episodeBaseControllerList({ seriesId });
  ...
  const handleUpdateEpisode = async (dto: UpdateEpisodeDto) => {
    try {
      await EpisodesService.episodeBaseControllerUpdate(dto);
  ...
  const handleUnbindTorrent = async (
    torrentId: string,
    seriesId: string,
