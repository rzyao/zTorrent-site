    ...
    <div className="mb-6">
      <div className="mb-3 flex items-start justify-between">
        <div className="flex-1">
          ...
          {loading && <p className="text-sm text-gray-500">{t('torrents.loading')}</p>}
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        {data.isFree && (
          <div className="ml-2 flex items-center gap-1 text-sm text-green-400">
            ...
            <span>{t('torrents.freeUntil', { date: data.promotionEnd })}</span>
          </div>
        )}
      </div>

      <div className="mb-3 flex flex-wrap items-center gap-3 text-xs text-gray-400">
        <span className="text-gray-200">{t('torrents.category')}</span>
        <span className="text-gray-200">{data.category}</span>
        <span className="text-gray-600">|</span>
        <span className="text-gray-200">{t('torrents.medium')}</span>
        <span className="text-gray-200">{data.medium}</span>
        <span className="text-gray-600">|</span>
        <span className="text-gray-200">{t('torrents.videoCodec')}</span>
        <span className="text-gray-200">{data.videoCodec}</span>
        <span className="text-gray-600">|</span>
        <span className="text-gray-200">{t('torrents.resolution')}</span>
        <span className="text-gray-200">{data.standard}</span>
        <span className="text-gray-600">|</span>
        <span className="text-gray-200">{t('torrents.audioCodec')}</span>
        <span className="text-gray-200">{data.audioCodec}</span>
        <span className="text-gray-600">|</span>
        <span className="text-gray-200">{t('torrents.team')}</span>
        <span className="text-gray-200">{data.productionTeam}</span>
        <span className="text-gray-600">|</span>
        <span className="text-gray-200">{t('torrents.size')}</span>
        <span className="text-gray-200">{formatSize(data.size)}</span>
        <span className="text-gray-600">|</span>
        <span className="text-gray-200">{t('torrents.uploadDate')}</span>
        <span className="text-gray-200">{data.uploadDate}</span>
        <span className="text-gray-600">|</span>
        <span className="text-gray-200">{t('torrents.uploader')}</span>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <DownloadButton
          ...
          labels={{
            idle: t('torrents.downloadTorrent'),
            loading: t('torrents.downloading'),
            success: t('torrents.downloaded'),
          }}
        />
        <ToggleButton
          ...
          tooltip={favorite.isFavorite ? t('torrents.unfavorite') : t('torrents.clickToFavorite')}
        >
          {favorite.isFavorite ? t('torrents.favorited') : t('torrents.favorite')}
        </ToggleButton>
