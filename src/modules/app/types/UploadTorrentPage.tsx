...
// --- 更新后的判断函数 ---
function getResolutionStandard(
  width: string,
  height: string,
...
export function parseMediaInfo(mediaInfoText: string): MediaInfoResult {
  ...
  lines.forEach((line) => {
    ...
    if (sectionMatch) {
      ...
      const normalizedSection = (sectionName.charAt(0).toUpperCase() +
        sectionName.slice(1).toLowerCase()) as keyof MediaInfoResult;
  ...
  if (result.Video) {
    ...
    const rawWidth = videoInfo.width
      ? videoInfo.width.replace(" pixels", "").replace(/[\s,]/g, "")
      : null;
    const rawHeight = videoInfo.height
      ? videoInfo.height.replace(" pixels", "").replace(/[\s,]/g, "")
      : null;
    ...
    if (rawWidth && rawHeight) {
      ...
      // 2. 计算 Standard
      videoInfo.standard = getResolutionStandard(
        rawWidth,
        rawHeight,
        videoInfo.scanType
      );
