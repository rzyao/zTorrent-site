 ...
 */
export const useBanDaysLogic = () => {
  // --- 状态管理 ---
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState<BanDay[]>([]);
  const [total, setTotal] = useState(0);
  const [query, setQuery] = useState<BanDayQuery>(DEFAULT_QUERY);
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      setData((res.data?.items as BanDay[]) || []);
      setTotal(res.data?.total || 0);
    } catch (error) {
      console.error("加载封禁时长列表失败:", error);
    } finally {
      setLoading(false);
    }
  }, [query]);
  ...
  // --- 列定义 ---
  const columns = useMemo<Column<BanDay>[]>(
    () => [
      {
        key: "sort",
        title: "排序",
