...
export function CreateRouteModal({
  ...
  // �?Modal 打开�?initialLayout 变化时，重置表单并设置初始�?
  useEffect(() => {
    if (open) {
      form.resetFields();
      form.setFieldsValue({
      ...
      >
        ...
        <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2">
          ...
          <Form.Item
            label="路由路径 (path)"
            name="path"
            rules={[{ required: true, message: "请输入路由路�? }]}
            tooltip="访问路径，如 /admin/users 或相对路�?users"
          >
            <Input placeholder="/admin/users �?users" />
          </Form.Item>
        ...
        <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2">
          <Form.Item label="渲染组件" name="component">
            <Select showSearch allowClear placeholder="选择组件 (可�?" optionFilterProp="children">
              {componentList.map((c) => (
          ...
          <Form.Item label="重定�? name="redirect">
            <Input placeholder="可�? /admin/users/list" />
          </Form.Item>

          <Form.Item label="排序权重" name="sortOrder" tooltip="数字越小越靠�?>
            <InputNumber min={0} step={10} style={{ width: "100%" }} />
          </Form.Item>

          <Form.Item label="图标" name="icon" tooltip="格式: Lucide:Home �?Antd:DashboardOutlined">
        ...
        <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2">
          <Form.Item label="访问权限" name="permissions" tooltip="输入权限Key并回�?>
          ...
          <div className="flex items-end gap-6 pb-2">
            ...
            <Form.Item label="启用状�? name="isEnabled" valuePropName="checked" className="mb-0">
              <Switch />
            </Form.Item>
