...
export const CreateSettingModal: React.FC<CreateSettingModalProps> = ({
  ...
  const [group, setGroup] = useState<SettingGroup>(initialGroup || "site");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // onSubmit in parent handles notification usually
    }
  };
    ...
    >
      <div className="space-y-4">
        <div className="grid gap-2">
          <Label htmlFor="group" className="required">
            分组
          </Label>
          <StandardSelect
            value={group}
        ...
        <div className="grid gap-2">
          <Label htmlFor="suffix" className="required">
            键名后缀
          </Label>
          <Input
            id="suffix"
            ...
            placeholder="例如：title 或 mail.smtp.host"
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            <Switch id="mutable" checked={mutable} onCheckedChange={setMutable} />
            <Label htmlFor="mutable" className="cursor-pointer">
              可运行时修改
            </Label>
          </div>
        </div>
