...
export function useForumBountyCancel(params: { status?: BountyStatus; page?: number; limit?: number }) {
  const page = params.page ?? 1;
  const limit = params.limit ?? 20;
  const cancelRequestStatus =
    params.status && params.status !== "all" ? (params.status as AdminListTopicBountyCancelRequestsDto.cancelRequestStatus) : undefined;
  ...
  const review = async (topicId: string, action: "approve" | "reject", note?: string) => {
    const payload: AdminReviewTopicBountyCancelRequestDto = {
      topicId,
      action: action === "approve" ? AdminReviewTopicBountyCancelRequestDto.action.APPROVE : AdminReviewTopicBountyCancelRequestDto.action.REJECT,
      note,
    };
