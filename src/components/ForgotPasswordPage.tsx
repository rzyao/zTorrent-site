        ...
        <div className="w-full max-w-md">
          <div className="bg-black/60 backdrop-blur-md rounded-lg p-8 md:p-10 border border-gray-800">
            ...
            {step !== 'success' && (
              <div className="flex items-center justify-center gap-2 mb-8">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${
                  step === 'email' ? 'bg-[#00A8E1] text-white' : 
                  step === 'verify' || step === 'reset' ? 'bg-green-500 text-white' : 
                  'bg-gray-700 text-gray-400'
                }`}>
                  1
                </div>
                <div className={`w-12 h-0.5 ${
                  step === 'verify' || step === 'reset' ? 'bg-green-500' : 'bg-gray-700'
                }`} />
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${
                  step === 'verify' ? 'bg-[#00A8E1] text-white' : 
                  step === 'reset' ? 'bg-green-500 text-white' : 
                  'bg-gray-700 text-gray-400'
                }`}>
                  2
                </div>
                <div className={`w-12 h-0.5 ${
                  step === 'reset' ? 'bg-green-500' : 'bg-gray-700'
                }`} />
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${
                  step === 'reset' ? 'bg-[#00A8E1] text-white' : 'bg-gray-700 text-gray-400'
                }`}>
                  3
                </div>
              </div>
            ...
            {step === 'email' && (
              <>
                <div className="text-center mb-8">
                  <h1 className="text-white text-3xl mb-2">找回密码</h1>
                ...
                <form onSubmit={handleSendCode} className="space-y-5">
                  <div className="space-y-2">
                    <label className="text-white text-sm">注册邮箱</label>
            ...
            {step === 'verify' && (
              <>
                <div className="text-center mb-8">
                  <h1 className="text-white text-3xl mb-2">验证身份</h1>
                  <p className="text-gray-400 text-sm">
                    验证码已发送至
                  </p>
                  <p className="text-[#00A8E1] text-sm mt-1">{email}</p>
                </div>

                <form onSubmit={handleVerifyCode} className="space-y-5">
                  <div className="space-y-2">
                    <label className="text-white text-sm">验证码</label>
                    <div className="flex gap-2">
                      <Input
                        ...
                        maxLength={6}
                        required
                        className="flex-1 bg-gray-900/50 border-gray-700 text-white text-center text-2xl tracking-widest px-4 py-6 rounded-md focus:border-[#00A8E1] focus:ring-[#00A8E1] placeholder:text-gray-500 placeholder:text-base placeholder:tracking-normal"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2 text-gray-400">
                      ...
                      <span>
                        {countdown > 0 ? `${countdown}秒后可重发` : '验证码已过期'}
                      </span>
                    </div>
                    <button
                      type="button"
                      ...
                      className={`${
                        countdown > 0
                          ...
                          : 'text-[#00A8E1] hover:text-[#00A8E1]/80'
                      } transition-colors`}
                    >
                      重新发送
                    </button>
            ...
            {step === 'reset' && (
              <>
                <div className="text-center mb-8">
                  <h1 className="text-white text-3xl mb-2">设置新密码</h1>
                  <p className="text-gray-400 text-sm">
                    请设置一个安全的新密码
                  </p>
                </div>
                ...
                <form onSubmit={handleResetPassword} className="space-y-5">
                  <div className="space-y-2">
                    <label className="text-white text-sm">新密码</label>
                    <div className="relative">
                  ...
                  <div className="space-y-2">
                    <label className="text-white text-sm">确认新密码</label>
                    <div className="relative">
                      <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                      <Input
                        type="password"
            ...
            {step === 'success' && (
              <>
                <div className="text-center py-8">
                  ...
                  <h1 className="text-white text-3xl mb-3">密码重置成功</h1>
