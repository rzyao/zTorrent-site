...
import { purchaseItem, getOrderDetail, type PurchaseRequest, type PurchaseResult, type OrderDetail } from '../api/custom/store';
...
export function usePurchase() {
  ...
  const purchase = useCallback(async (req: PurchaseRequest, opts: PurchaseOptions = {}): Promise<PurchaseSuccessPayload> => {
    ...
    try {
      ...
      if (opts.refreshBonus) {
        try {
          ...
          const rawBal = typeof b?.balance === 'string' ? parseFloat(b.balance as string) : Number(b?.balance ?? 0);
          const rawLocked = typeof b?.lockedBalance === 'string' ? parseFloat(b.lockedBalance as string) : Number(b?.lockedBalance ?? 0);
          bonusInfo = { balance: rawBal, lockedBalance: rawLocked };
          const o = await getBonusOverview();
          const bal = typeof o.balance === 'string' ? parseFloat(o.balance as string) : Number(o.balance);
          const earned = typeof o.totalEarned === 'string' ? parseFloat(o.totalEarned as string) : Number(o.totalEarned);
          const spent = typeof o.totalSpent === 'string' ? parseFloat(o.totalSpent as string) : Number(o.totalSpent);
          overviewInfo = { balance: bal, totalEarned: earned, totalSpent: spent };
        } catch { }
      }

      if (typeof opts.onRefresh === 'function') {
        try { await opts.onRefresh(); } catch { }
      }
